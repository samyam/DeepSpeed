...
#pragma unroll
        ...
        for (int i = 0; i < reg_count; i++) {
            ...
            if (group_index < group_size) {
                ...
                q_data_int[0].x = round(q_data[0].x * q_scale);
                q_data_int[0].y = round(q_data[0].y * q_scale);
                q_data_int[1].x = round(q_data[1].x * q_scale);
                q_data_int[1].y = round(q_data[1].y * q_scale);
...
#pragma unroll
    ...
    for (int i = 0; i < reg_count; i++) {
        ...
        if (group_index < group_size) {
            ...
            float4 q_data_int;
            q_data_int.x = round(q_data.x * q_scale);
            q_data_int.y = round(q_data.y * q_scale);
            q_data_int.w = round(q_data.w * q_scale);
            q_data_int.z = round(q_data.z * q_scale);
...
#pragma unroll
        ...
        for (int i = 0; i < reg_count; i++) {
            ...
            if (group_index < group_size) {
                ...
                q_data_int[0].x = round((q_data[0].x - min) * q_scale_inv);
                q_data_int[0].y = round((q_data[0].y - min) * q_scale_inv);
                q_data_int[1].x = round((q_data[1].x - min) * q_scale_inv);
                q_data_int[1].y = round((q_data[1].y - min) * q_scale_inv);
...
#pragma unroll
    ...
    for (int i = 0; i < reg_count; i++) {
        ...
        if (group_index < group_size) {
            ...
            float4 q_data_int;
            q_data_int.x = round((q_data.x - min) * q_scale_inv);
            q_data_int.y = round((q_data.y - min) * q_scale_inv);
            q_data_int.w = round((q_data.w - min) * q_scale_inv);
            q_data_int.z = round((q_data.z - min) * q_scale_inv);
