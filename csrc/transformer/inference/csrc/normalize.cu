                                          ...
                                          int row_stride)
{
    ...
    while (input_id < row_stride) {
        ...
        res_add[input_id + row * row_stride] = inp_reg[k];
        sum += inp_reg[k++];
        input_id += iteration_stride;
    }
    for (int i = 1; i < 32; i *= 2) sum += g.shfl_down(sum, i);
                                          ...
                                          int row_stride)
{
    ...
    while (input_id < row_stride) {
        ...
        res_add_cast[input_id + row * row_stride] = inp_reg[k++];
        sum += inp_f.x + inp_f.y;
        input_id += iteration_stride;
    }
    for (int i = 1; i < 32; i *= 2) sum += g.shfl_down(sum, i);
                                       ...
                                       cudaStream_t stream)
{
    ...
    fused_residual_layer_norm<<<grid_dim, block_dim, 0, stream>>>(
        norm, res_add, vals, residual, bias, gamma, beta, epsilon, hidden_dim);
}
                                        ...
                                        cudaStream_t stream)
{
    ...
    fused_residual_layer_norm<<<grid_dim, block_dim, 0, stream>>>(
        norm, res_add, vals, residual, bias, gamma, beta, epsilon, hidden_dim / 2);
}
